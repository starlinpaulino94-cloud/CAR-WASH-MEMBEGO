-- =============================================================================
-- PARCHE 0023 (editor SQL de Supabase) · Inspección de recepción y entrega
-- =============================================================================
...
do $do$ begin
  create type app.inspection_stage as enum
('recepcion', 'entrega');
...
exception when duplicate_object then null; end $do$;
do $do$ begin
  create type app.damage_severity as enum
('leve', 'moderado', 'grave');
exception when duplicate_object then null; end $do$;
...
create index if not exists vehicle_inspections_order_idx on public.vehicle_inspections (work_order_id);
create index if not exists vehicle_inspections_company_idx on public.vehicle_inspections (company_id, created_at desc);

drop trigger if exists vehicle_inspections_touch on public.vehicle_inspections;
create trigger vehicle_inspections_touch
...
create index if not exists inspection_damages_inspection_idx on public.inspection_damages (inspection_id);
...
drop trigger if exists vehicle_inspections_freeze on public.vehicle_inspections;
...
drop trigger if exists inspection_damages_freeze on public.inspection_damages;
...
drop policy if exists inspection_damages_select on public.inspection_damages;
create policy inspection_damages_select on public.inspection_damages
  for select to authenticated using (app.belongs_to_tenant(company_id));

drop policy if exists inspection_damages_write on public.inspection_damages;
