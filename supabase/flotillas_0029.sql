-- =============================================================================
-- PARCHE 0029 (editor SQL de Supabase) · Flotillas y contratos corporativos
-- =============================================================================
-- Ejecuta este script COMPLETO en el editor SQL (Production), DESPUÉS del
-- parche 0028 (crédito y cuentas por cobrar), del que depende: lo consolidado
-- se factura siempre a crédito.
...
create index if not exists fleets_company_idx on public.fleets (company_id) where is_active;
create index if not exists fleets_customer_idx on public.fleets (customer_id);

drop trigger if exists fleets_touch on public.fleets;
create trigger fleets_touch before update on public.fleets
  for each row execute function app.touch_updated_at();

-- ------------------------------------------------- Vehículos de la flotilla
alter table public.vehicles add column if not exists fleet_id uuid;

-- `add constraint` no admite `if not exists`: se captura el duplicado.
-- `set null (columna)` desde PostgreSQL 15: sin acotarlo, borrar una flotilla
-- intentaría anular también company_id, que es NOT NULL.
do $do$ begin
...
create index if not exists vehicles_fleet_idx on public.vehicles (fleet_id) where fleet_id is not null;
...
create unique index if not exists fleet_rates_specific_unique
  on public.fleet_rates (fleet_id, service_id, vehicle_category)
  where vehicle_category is not null;
create unique index if not exists fleet_rates_generic_unique
  on public.fleet_rates (fleet_id, service_id)
  where vehicle_category is null;

create index if not exists fleet_rates_fleet_idx on public.fleet_rates (fleet_id);

drop trigger if exists fleet_rates_touch on public.fleet_rates;
...
alter table public.work_orders
  ...
  add column if not exists consolidated_invoice_id uuid;
...
create index if not exists work_orders_fleet_pending_idx
...
drop policy if exists fleet_rates_select on public.fleet_rates;
