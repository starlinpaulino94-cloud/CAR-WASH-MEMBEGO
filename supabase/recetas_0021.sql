-- =============================================================================
-- PARCHE 0021 (editor SQL de Supabase) · Recetas de insumos y costo real
-- =============================================================================
-- Ejecuta este script COMPLETO en el editor SQL (Production), DESPUÉS de los
-- parches 0019 y 0020. Es idempotente. Habilita el botón de Receta en
-- Ventas → Servicios y el consumo automático al entregar cada orden.
-- =============================================================================
...
alter table public.products
  add column if not exists stock_frac numeric(8,3) not null default 0
...
create index if not exists service_recipes_service_idx on public.service_recipes (service_id);

drop trigger if exists service_recipes_touch on public.service_recipes;
...
drop policy if exists service_recipes_select on public.service_recipes;
...
drop policy if exists service_recipes_write on public.service_recipes;
...
create index if not exists service_consumptions_order_idx   on public.service_consumptions (work_order_id);
create index if not exists service_consumptions_company_idx on public.service_consumptions (company_id, created_at desc);
