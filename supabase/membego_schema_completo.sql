...
--   3. Pulsa "Run". Se ejecuta como una sola transacción implícita del editor.
--
-- REQUISITOS
--   * Ejecútalo sobre un proyecto NUEVO/VACÍO. Usa `create type`, `create table`
--     y `create trigger` sin `if not exists`, así que volver a correrlo sobre un
--     esquema ya creado dará errores de "ya existe" (es deliberado: falla antes
--     de pisar datos). Para reaplicar, primero borra el esquema.
--   * No incluye el "shim" de auth de las pruebas locales: Supabase ya trae el
--     esquema `auth`, `auth.users`, `auth.uid()` y los roles anon/authenticated/
