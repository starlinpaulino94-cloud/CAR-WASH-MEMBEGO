-- =============================================================================
-- 0030 · Turnos, asistencia y nómina
-- =============================================================================
-- El personal es el mayor costo de un car wash y era lo único que el sistema no
-- sabía calcular. Las comisiones se generaban al entregar, pero nadie las
-- pagaba: `is_paid` nunca cambiaba. El sueldo no existía en ninguna tabla. Los
-- adelantos —que en la práctica salen de la gaveta cada semana— no dejaban
-- rastro. Y no había forma de saber quién llegó ni a qué hora.
--
-- Este módulo cierra el ciclo completo:
--
--   · Datos de pago en el perfil (tipo, sueldo, tarifa por hora) protegidos por
--     un guardia. De paso tapa un agujero que ya existía: `profiles_update_self`
--     dejaba a cualquiera editar su propia ficha, y con ella su comisión.
--   · work_shifts: el turno planificado, sin solapes.
--   · attendance_records: la entrada y la salida reales, con la tardanza medida
--     contra el turno de ese día.
--   · payroll_advances: el adelanto que sale de la caja y se descuenta después.
--   · payroll_periods + payroll_items: la nómina, en tres estados —borrador,
--     aprobada, pagada—. Pagarla marca las comisiones como pagadas, salda los
--     adelantos y registra la salida de caja.
--
-- Una comisión o un adelanto quedan AMARRADOS a la partida de nómina que los
-- recoge. Es lo que impide pagarlos dos veces, que es el error que de verdad
-- cuesta dinero.
-- =============================================================================
...
alter table public.profiles
  add column payroll_type      app.payroll_type not null default 'solo_comision',
  add column base_salary_cents bigint not null default 0 check (base_salary_cents >= 0),
...
create index work_shifts_profile_idx on public.work_shifts (profile_id, starts_at desc);
create index work_shifts_company_idx  on public.work_shifts (company_id, starts_at desc);
...
create index attendance_profile_idx on public.attendance_records (profile_id, checked_in_at desc);
create index attendance_company_idx on public.attendance_records (company_id, checked_in_at desc);
...
create unique index attendance_one_open_per_profile
...
create index payroll_advances_pending_idx on public.payroll_advances (company_id, profile_id)
...
create index payroll_periods_company_idx on public.payroll_periods (company_id, period_from desc);
...
alter table public.commissions
  add column payroll_item_id uuid,
...
create index commissions_unlinked_idx on public.commissions (company_id, profile_id, earned_on)
  where payroll_item_id is null;
