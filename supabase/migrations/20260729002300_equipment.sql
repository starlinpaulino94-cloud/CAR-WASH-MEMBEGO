...
create type app.equipment_status as enum ('operativo', 'mantenimiento', 'fuera_servicio', 'retirado');
create type app.maintenance_kind as enum ('preventivo', 'correctivo');
create type app.maintenance_status as enum ('abierta', 'completada', 'cancelada');
...
create index equipment_company_idx on public.equipment (company_id, status);
create index equipment_due_idx on public.equipment (company_id, next_service_at)
...
create index maintenance_orders_equipment_idx on public.maintenance_orders (equipment_id, started_at desc);
create index maintenance_orders_open_idx on public.maintenance_orders (company_id)
  where status = 'abierta';

create trigger maintenance_orders_touch before update on public.maintenance_orders
  for each row execute function app.touch_updated_at();
