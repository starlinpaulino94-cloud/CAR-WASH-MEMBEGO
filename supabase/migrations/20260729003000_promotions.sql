-- =============================================================================
-- 0032 · Promociones y descuentos controlados
-- =============================================================================
-- El descuento era un número libre que ponía el cliente de la aplicación. La
-- única barrera era un CHECK por línea: «no descuentes más que el importe de la
-- línea». Es decir, cualquiera con acceso al punto de venta podía dejar una
-- factura en cero, y nadie se enteraba. Es el agujero más barato de explotar de
-- todo el sistema.
--
-- Este módulo pone las dos piezas que faltaban:
--
--   · Un TECHO al descuento manual. `companies.max_manual_discount_bps` limita
--     lo que puede rebajar quien atiende. La propiedad y la administración se
--     lo saltan —alguien tiene que poder decidir—, pero queda en la bitácora.
--   · Promociones DE VERDAD: un código con sus reglas —vigencia, día de la
--     semana, compra mínima, alcance, tope de usos— y el descuento lo calcula
--     el servidor. La pantalla solo previsualiza.
--
-- promotion_redemptions es lo que hace cumplir los topes: sin registrar cada
-- canje, «máximo 100 usos» es un adorno.
--
-- COMPATIBILIDAD: `max_manual_discount_bps` nace en 10000 (100 %), o sea, sin
-- límite. Aplicar la migración no cambia lo que hoy se puede hacer; el techo se
-- baja cuando el dueño quiera, desde Configuración › Empresa.
-- =============================================================================
...
alter table public.companies
  add column max_manual_discount_bps integer not null default 10000
...
create index promotions_active_idx on public.promotions (company_id) where is_active;
...
create index redemptions_promotion_idx on public.promotion_redemptions (promotion_id);
create index redemptions_customer_idx on public.promotion_redemptions (customer_id)
