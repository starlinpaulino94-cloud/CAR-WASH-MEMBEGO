...
-- 0031 · Multisucursal
-- =============================================================================
-- La tabla `branches` existía desde el primer día y casi todo lleva su
-- `branch_id`, pero nada lo usaba: no había forma de dar de alta una sucursal
-- desde la aplicación, y el aislamiento llegaba solo hasta la empresa. Un
-- cajero de la sucursal del centro veía —y podía tocar— la caja, las órdenes y
-- las facturas de la sucursal de la autopista.
--
-- Este módulo añade:
--
--   · Alta y edición de sucursales por RPC, con las reglas que el índice único
--     por sí solo no puede dar: una sola principal, y no se desactiva una
--     sucursal con la caja abierta ni la última que queda en pie.
--   · profiles.branch_scope: «todas» o «su sucursal». Es lo que convierte el
--     branch_id en una frontera real.
--   · Políticas RESTRICTIVE de alcance sobre las tablas operativas. Restrictive
--     se combina con AND sobre las permisivas que ya existen, así que el filtro
--     se añade sin reescribir ni una sola política anterior.
--   · management_report acepta sucursal: el dueño compara sus locales.
--
-- COMPATIBILIDAD: `branch_scope` nace en «todas» para TODO EL MUNDO. Aplicar
-- esta migración no le esconde a nadie nada de lo que ve hoy. El aislamiento se
-- activa empleado por empleado desde Configuración › Sucursales. Es deliberado:
-- estrechar el alcance de golpe, a mitad de un turno, sería peor que el
-- problema que resuelve.
--
-- FUERA DE ALCANCE, a propósito: la existencia por sucursal. `products` es una
-- fila por empresa (unique company_id, code) con un `stock` único, así que un
-- traspaso entre locales exigiría rehacer el inventario. Se deja para su propia
-- fase en vez de dejarlo a medias aquí.
-- =============================================================================

create type app.branch_scope as enum ('sucursal', 'todas');

alter table public.profiles
  add column branch_scope app.branch_scope not null default 'todas';
