-- =============================================================================
-- 0033 · Notificaciones y avisos
-- =============================================================================
-- Dos cosas que el sistema sabía y no decía:
--
--   · Al cliente. El vehículo queda listo y nadie se lo avisa; el carro se
--     queda en la bahía ocupándola hasta que alguien se acuerda de llamar.
--   · Al dueño. El producto bajó del mínimo, la cuenta venció hace una semana,
--     el equipo lleva dos meses sin mantenimiento. Todo estaba en la base, pero
--     había que ir a buscarlo pantalla por pantalla.
--
-- Este módulo añade una BANDEJA DE SALIDA. Los avisos se generan solos y se
-- quedan ahí, en estado «pendiente», hasta que se envían o se descartan.
--
-- Por qué una bandeja y no un envío directo: mandar un WhatsApp exige un
-- proveedor contratado que hoy no existe. Encolar el aviso sí se puede hacer
-- bien desde ya, y el día que haya proveedor solo cambia quién vacía la cola.
-- Mientras tanto la pantalla ofrece el enlace de WhatsApp con el texto ya
-- escrito: un clic desde el teléfono del mostrador y listo.
--
-- `dedupe_key` es la pieza que hace esto usable: sin ella, un producto bajo
-- mínimo generaría un aviso por cada vez que alguien abre la pantalla, y en dos
-- días nadie volvería a mirar la bandeja.
-- =============================================================================
...
create index notifications_kind_idx on public.notifications (company_id, kind, created_at desc);
