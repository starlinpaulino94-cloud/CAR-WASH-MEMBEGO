-- =============================================================================
-- SSO Membego · al cambiar de empresa, el perfil cambia también de sucursal
-- =============================================================================
-- SÍNTOMA (tras estrenar la auto-vinculación 20260812210000): «La sucursal
-- <uuid> no pertenece a la empresa <uuid>» (23514) al entrar por SSO a una
-- empresa nueva con un usuario que YA tenía perfil en otra.
--
-- CAUSA: el upsert movía `profiles.company_id` a la empresa del token pero
-- dejaba `profiles.branch_id` apuntando a la sucursal de la empresa ANTERIOR.
-- El trigger `profiles_branch_belongs_to_company` hace exactamente su trabajo
-- y lo rechaza — el guardián está bien; el que olvidaba la sucursal era el SSO.
--
-- ARREGLO: al cambiar de empresa, el perfil pasa a la SUCURSAL PRINCIPAL de la
-- nueva (la auto-vinculación siempre crea una). Si el usuario entra a la MISMA
-- empresa de siempre, su sucursal asignada NO se toca: esa asignación es del
-- operador local y el SSO no tiene por qué pisarla.
--
-- Idempotente (create or replace, misma firma de 5 argumentos).
-- =============================================================================
...
comment on function public.membego_sso_upsert_user is
  'SSO Membego con AUTO-VINCULACIÓN (20260812210000) y cambio de sucursal al cambiar de empresa (20260812220000): el perfil siempre queda en una sucursal de SU empresa; la asignación local solo se toca al cruzar de empresa.';
