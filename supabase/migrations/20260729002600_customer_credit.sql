-- =============================================================================
-- 0028 · Crédito de clientes y cuentas por cobrar
-- =============================================================================
-- El punto de venta ya aceptaba «crédito» como forma de pago, pero nadie
-- validaba nada: la factura se emitía, la orden se marcaba PAGADA y el importe
-- entraba a la caja como si hubiese entrado dinero. Fiar era, literalmente,
-- regalar el servicio y descuadrar el arqueo.
--
-- Este módulo cierra el circuito:
--
--   · customers gana cupo, plazo y autorización de crédito, protegidos por un
--     guardia: esas tres columnas no se editan con un UPDATE cualquiera, solo
--     con set_customer_credit() y solo desde los roles que mandan en el dinero.
--   · receivables: una cuenta por cobrar por cada factura fiada, con su
--     vencimiento y su saldo. receivable_payments guarda los abonos.
--   · create_invoice deja de mentir: la parte fiada NO entra a la caja, abre
--     la cuenta por cobrar, y la orden queda «pendiente» o «parcial» según lo
--     que se haya cobrado de verdad.
--   · Antes de fiar se comprueba cupo disponible y mora. Un cliente con una
--     factura vencida no se lleva otro lavado a crédito.
--   · annul_invoice cancela la cuenta por cobrar de la factura anulada.
--
-- Igual que en compras (0020), la escritura de saldos pasa siempre por RPC:
-- las tablas solo se leen. Lo que mueve dinero se valida en un solo sitio.
-- =============================================================================
...
alter table public.customers
  ...
  add column credit_limit_cents bigint  not null default 0 check (credit_limit_cents >= 0),
  -- Días de plazo desde la emisión. 0 = vence el mismo día.
  add column credit_terms_days  integer not null default 0
    check (credit_terms_days between 0 and 365);

create index customers_credit_idx on public.customers (company_id)
...
create index receivables_customer_idx on public.receivables (company_id, customer_id)
  where status = 'pendiente';
-- Índice de la vejez de saldos: el listado natural es «lo más vencido primero».
create index receivables_due_idx on public.receivables (company_id, due_on)
  where status = 'pendiente';
...
create index receivable_payments_receivable_idx
