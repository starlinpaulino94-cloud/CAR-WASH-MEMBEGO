-- =============================================================================
-- 0034 · Notas de crédito parciales y reinicio de contraseña
-- =============================================================================
-- Cierra los dos últimos huecos que quedaban marcados «en preparación».
--
-- NOTAS DE CRÉDITO PARCIALES. Hasta ahora anular era todo o nada: annul_invoice
-- devuelve la factura entera. En la calle pasa lo contrario casi siempre —se
-- cobró un encerado que no se hizo, se facturaron dos aromatizantes y se
-- entregó uno—, y la salida era anular la factura completa y volver a emitirla.
-- Eso quema un NCF, ensucia el histórico y descuadra la caja dos veces.
--
-- credit_note_invoice acredita CANTIDADES de líneas concretas:
--
--   · Cada línea recuerda cuánto se le ha acreditado ya, así que dos notas
--     parciales sobre la misma línea no pueden pasarse de lo facturado.
--   · El inventario vuelve solo por las unidades acreditadas.
--   · Si la venta fue a crédito, lo acreditado baja primero la cuenta por
--     cobrar; solo lo que exceda el saldo se devuelve en efectivo. Devolver
--     dinero de algo que nunca se cobró sería regalarlo dos veces.
--   · Cuando lo acreditado alcanza el total, la factura queda anulada: es lo
--     mismo que anularla, pero por acumulación.
--
-- REINICIO DE CONTRASEÑA. Un empleado que olvida su clave no tenía forma de
-- volver a entrar: create_employee solo sirve para altas. Con el mismo techo de
-- rol que el alta, para que un administrador no le cambie la clave al dueño.
-- =============================================================================

-- Cuánto se ha acreditado ya, en la factura y en cada una de sus líneas.
alter table public.invoices
  add column credited_cents bigint not null default 0 check (credited_cents >= 0);

alter table public.invoice_items
  add column credited_quantity integer not null default 0 check (credited_quantity >= 0),
  add constraint invoice_items_credited_within_line
    check (credited_quantity <= quantity);
