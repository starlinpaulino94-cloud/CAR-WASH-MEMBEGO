-- =============================================================================
-- PARCHE 0024 (editor SQL de Supabase) · Control de calidad con checklist
-- =============================================================================
...
do $do$ begin
  create type app.qc_result as enum
('aprobado', 'rechazado');
...
create index if not exists qc_checklist_items_company_idx on public.qc_checklist_items (company_id, sort_order);

drop trigger if exists qc_checklist_items_touch on public.qc_checklist_items;
create trigger qc_checklist_items_touch
...
create index if not exists qc_reviews_order_idx   on public.qc_reviews (work_order_id, attempt);
create index if not exists qc_reviews_company_idx on public.qc_reviews (company_id, created_at desc);
create index if not exists qc_reviews_washer_idx  on public.qc_reviews (washer_id) where result = 'rechazado';

create table if not exists public.qc_review_results (
  id          uuid primary key default gen_random_uuid(),
  company_id  uuid not null references public.companies(id) on delete cascade,
...
create index if not exists qc_review_results_review_idx on public.qc_review_results (review_id);
...
drop policy if exists qc_reviews_select on public.qc_reviews;
create policy qc_reviews_select on public.qc_reviews
  for select to authenticated using (app.belongs_to_tenant(company_id));
drop policy if exists qc_review_results_select on public.qc_review_results;
