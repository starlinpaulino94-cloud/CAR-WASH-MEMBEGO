-- =============================================================================
-- PARCHE 0027 (editor SQL de Supabase) · Reclamos e incidentes
-- =============================================================================
...
create index if not exists claims_company_time_idx on public.claims (company_id, created_at desc);
create index if not exists claims_open_idx on public.claims (company_id, status)
  where status in ('abierto', 'en_revision');
create index if not exists claims_order_idx on public.claims (work_order_id) where work_order_id is not null;
create index if not exists claims_customer_idx on public.claims (customer_id) where customer_id is not null;
...
drop trigger if exists claim_events_no_mutation on public.claim_events;
