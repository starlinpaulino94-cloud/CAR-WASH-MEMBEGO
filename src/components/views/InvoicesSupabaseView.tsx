    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-line pb-4">
        <div>
          <h2 className="text-xl font-bold text-strong flex items-center gap-2">
            <Receipt className="w-5 h-5 text-brand" /> Facturas y comprobantes
          </h2>
      ...
      {notice && (
        <div role="status" className="flex items-start gap-2 p-3 bg-success/40 border border-success/40 rounded-xl text-xs text-success">
          <FileMinus className="w-4 h-4 flex-shrink-0 text-success mt-0.5" />
          <div className="flex-1">{notice}</div>
          <button onClick={() => setNotice(null)} className="text-success hover:text-success font-bold px-1" aria-label="Descartar aviso">×</button>
        </div>
      )}
                ...
                return (
                  <tr key={inv.id} className={`hover:bg-surface-2/40 transition-colors ${inv.is_annulled ? 'opacity-60' : ''}`}>
                    <td className="p-3">
                      ...
                      {inv.is_annulled && (
                        <span className="text-xs bg-danger/20 text-danger border border-danger/30 px-1.5 py-0.5 rounded font-bold">
                          ANULADA
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-3">
                      <div className="flex gap-2 justify-end">
                        <button
                          ...
                          className="px-2 py-1.5 bg-brand hover:bg-brand text-on-accent rounded text-xs flex items-center gap-1 font-semibold"
                        >
                        ...
                        {!inv.is_annulled && !isCredit && (
                          <button
                            ...
                            className="px-2 py-1.5 bg-danger/30 text-danger hover:bg-danger hover:text-on-accent disabled:opacity-30 disabled:cursor-not-allowed rounded text-xs flex items-center gap-1 font-semibold border border-danger/30"
                          >
