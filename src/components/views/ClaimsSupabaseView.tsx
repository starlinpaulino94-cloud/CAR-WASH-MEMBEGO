...
const STATUS_TONE: Record<string, string> = {
  ...
  rechazado: 'bg-slate-700/60 text-slate-300'
};
...
export const ClaimsSupabaseView: React.FC = () => {
  ...
  if (phase !== 'ready') {
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <ViewHeader icon={<MessageSquareWarning className="w-5 h-5 text-indigo-400" />}
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <ViewHeader
        icon={<MessageSquareWarning className="w-5 h-5 text-indigo-400" />}
        title="Reclamos"
            ...
            <tbody className="divide-y divide-slate-800/60">
              {q.loading ? <SkeletonRows cols={6} />
                : q.rows.length === 0 ? (
                  ...
                  <tr key={c.id} className="hover:bg-slate-800/40 cursor-pointer"
                    ...
                    <td className="p-3">
                      ...
                      {c.customer_phone && <div className="text-xs text-slate-500">{c.customer_phone}</div>}
                    </td>
                    <td className="p-3 text-slate-300 whitespace-nowrap">
                      {KINDS.find(k => k.id === c.kind)?.label ?? c.kind}
                    </td>
                    <td className="p-3 text-slate-400 max-w-md truncate">{c.description}</td>
                    <td className="p-3 text-right tabular-nums whitespace-nowrap text-slate-300">
      ...
      {detail && (
        ...
        >
          ...
          <p className="text-sm text-slate-200 bg-slate-950/60 border border-slate-800 rounded-xl p-3">
            {detail.description}
          </p>
          ...
          {events.length > 0 && (
            <div className="space-y-2">
              <span className="text-sm font-semibold text-slate-400 uppercase">Seguimiento</span>
              <ul className="space-y-1.5 max-h-48 overflow-y-auto">
                {events.map(e => (
                  <li key={e.id} className="text-sm bg-slate-950/50 rounded-lg p-2.5 flex gap-2">
          ...
          {closed ? (
            <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-3 space-y-1.5 text-sm">
              <div><span className="text-slate-400">Resolución:</span>{' '}
                <span className="text-white">{detail.resolution}</span></div>
              {detail.root_cause && (
                <div><span className="text-slate-400">Causa raíz:</span>{' '}
                  <span className="text-white">{detail.root_cause}</span></div>
              )}
              <div><span className="text-slate-400">Costo asumido:</span>{' '}
                <span className="text-white tabular-nums">{formatCents(detail.cost_cents, symbol)}</span></div>
            </div>
          ) : (
            <>
              ...
              {canClose && (
                <div className="border-t border-slate-800 pt-4 space-y-3">
                  <span className="text-sm font-semibold text-slate-400 uppercase">Cerrar el reclamo</span>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => void close('resuelto')} disabled={busy}
                      className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 text-white font-bold text-sm rounded-xl flex items-center gap-2">
                      {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                      Resuelto
                    </button>
                    <button type="button" onClick={() => void close('rechazado')} disabled={busy}
                      className="px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-200 font-bold text-sm rounded-xl flex items-center gap-2">
