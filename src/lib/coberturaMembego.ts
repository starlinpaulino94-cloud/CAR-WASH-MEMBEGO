...
export function aplicarCobertura(params: {
  ...
  /**
   * Los servicios que SÍ puede pagar una membresía, para poder nombrarlos en el
  ...
  // Ninguna línea de la venta es un servicio que la membresía pueda pagar.
  //
  // AQUÍ NO SE ADIVINA NADA, y conviene saber por qué está escrito tan fuerte.
  // Hubo una versión que, al no encontrar servicio marcado, tomaba el precio
  // del incluible más caro como «lo que vale el plan» y cobraba la diferencia.
  // Parecía razonable y era falso: un servicio sin marcar es DESCONOCIDO, no
  // más caro. Un cliente con plan que cubría su lavado entero acabó pagando una
  // diferencia inventada, explicada además con un motivo —la categoría del
  // vehículo— que su propia ficha desmentía.
  //
  // Un cobro inventado es peor que un cobro completo explicado: el completo se
  // discute en el mostrador y se corrige; el inventado tiene pinta de correcto
  // y se cobra mil veces. Si falta la marca, se dice y no se toca la factura.
  if (candidatas.length === 0) {
    const hayMembresiaViva = membresias.some(m =>
      m.coverage !== null && (m.coverage.unlimited || m.usesLeft > 0));
    const tieneServicio = lineas.some(l => l.serviceId !== null);
